...
for i in "${IP[@]}";do echo "$i"  >> ref下载log;  python 下载ref.py  -i "合并${num}000000-${num}999999有ref链接的aria2c文本.txt"  -r "$i" -d "/mnt/d/常用/vimeo/传统方法刷-下载后再处理数据-刷json/temp/ref" >> ref下载log; find /mnt/d/常用/vimeo/传统方法刷-下载后再处理数据-刷json/temp/ref -type f -size -555c -exec rm {} +;  for file in /mnt/d/常用/vimeo/传统方法刷-下载后再处理数据-刷json/temp/ref/*; do mv "$file" "${file}_"; done;done  
